...
################################################################################
# USO: R -e 'shiny::runApp(port=8222)'
# y con tu navegador abris http://localhost:8222
# cuando moves un parametro se actualiza el grafico (tarda unos segundos)
# 
# Algunas notas y mejoras de podemosaprender.org
################################################################################
...
server <- function(input, output) {
	...
	SIR_agent = function(
		...
		S = S_0
		I = I_0
		vS = numeric(0) # will be filled with the number of susceptibles in the population over time
		vI = numeric(0) # will be filled with the number of infectives in the population over time
		vtime = numeric(0) # will be filled with the time steps
...
# If a susceptible contacted at least one infective, they are infected
	...
	SIR_agent_erlang = function( #U: usando otra distribucion ... #TODO: generalizar en una sola!
			N # population size
			,I_0 # initial number infected
			,S_0 # initial number susceptible
			,gamma # recovery rate in days^{-1}
